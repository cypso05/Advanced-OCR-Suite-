...
// Unified save/export function
  // Updated handleSaveWithFormat function with proper notifications
  const handleSaveWithFormat = async (format = 'json') => {
    try {
      ...
      if (format === 'json') {
        // For JSON format, call parent's onSave
        if (onSave) {
          onSave(JSON.stringify(enhancedContent), format);
        }
        showNotification('Document saved successfully!', 'success');
        
        // Also export as JSON file
        await handleDocumentExport(format, exportData);
        ...
        // For other formats, just export to file
        await handleDocumentExport(format, exportData);
      ...
      <Grid item size={{ xs: 6, md: 2 }}>
        <FormControl fullWidth size="small" sx={{ 
          ...
          >
            ...
            // Add Font Family selector to the FormattingToolbar component
// Inside the Grid container, add this after the Font Size selector:
                  ...
                  {/* Bottom Right Handle */}
                    <Box
                      sx={{
                        ...
                        zIndex: 2,               // <-- Lowered so sidebar remains above
                      }}
                    >
                      <Box
                        onMouseDown={(e) => handleResizeStart(e, 'bottom-right')}
                        sx={{
                          pointerEvents: 'auto',   // <-- Only this receives events
                          width: '100%',
                          height: '100%',
                          cursor: 'nwse-resize',
                          backgroundColor: '#667eea',
...
const SaveDialog = ({ open, onClose, onSave, isSaving, selectedFormat, setSelectedFormat }) => (
  <Dialog 
    open={open} 
    onClose={onClose}
    maxWidth="sm"
  ...
  >
    ...
    <DialogContent sx={{ 
      ...
      <Typography variant="body1" sx={{ color: '#CBD5E0', mb: 3 }}>
        Choose how you want to save your document:
      </Typography>
      ...
      <Box sx={{ 
        ...
        <Typography variant="caption" sx={{ color: '#CBD5E0', fontWeight: 600 }}>
          {selectedFormat === 'json' ? 
            'This will save your document to the app storage and download a JSON file.' :
            selectedFormat === 'pdf' ? 
            'This will download your document as a PDF file.' :
            selectedFormat === 'docx' ? 
            'This will download your document as a Word file.' :
            selectedFormat === 'html' ? 
            'This will download your document as an HTML file.' :
            'This will download your document as a plain text file.'}
        </Typography>
      </Box>
    </DialogContent>
    ...
    <DialogActions sx={{ 
      ...
      >
        {isSaving ? 'Processing...' : selectedFormat === 'json' ? 'Save Document' : 'Export Document'}
      </Button>
    </DialogActions>
...
return (
  ...
  >
   ...
   {/* Export Format Menu */}
      ...
      >
        {[
          { format: 'json', label: 'Save as JSON', icon: <CloudDownload />, description: 'Save to app & download' },
          { format: 'pdf', label: 'Export as PDF', icon: <PictureAsPdf />, description: 'Download PDF file' },
          { format: 'docx', label: 'Export as Word', icon: <Description />, description: 'Download Word file' },
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, width: '100%' }}>
              <Box sx={{ color: 
                ...
                item.format === 'docx' ? '#4299E1' :
                item.format === 'txt' ? '#48BB78' :
                item.format === 'html' ? '#ED8936' : '#9F7AEA',
    ...
    {/* Enhanced Footer with Export Menu */}
    <DialogActions sx={{ 
      p: 1,
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
          ...
          <Box sx={{ position: 'relative' }}>
            <Button 
              onClick={() => setSaveDialogOpen(true)}  // Changed from direct save
              variant="contained" 
            ...
            >
              {isExporting ? 'Saving...' : 'Save Document'}
            </Button>
        ...
        <SaveDialog
          ...
          onSave={(format) => handleSaveWithFormat(format)}
